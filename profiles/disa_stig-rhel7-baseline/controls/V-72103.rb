...
FCHOWNAT_AUDIT_FIELDS = attribute(
  'fchownat_audit_fields',
  default: [
  ...
  description: "The fields that you use to audit the fchownat command."
)
...
The audit daemon must be restarted for the changes to take effect."
  ...
  describe.one do
    FCHOWNAT_AUDIT_FIELDS.first['arch'].each do |arch|
  ...
  describe auditd_rules.syscall("#{sys_call}").fields.flatten do
   it { should include FCHOWNAT_AUDIT_FIELDS.first['auid_pos'] }
  end
  describe.one do
    FCHOWNAT_AUDIT_FIELDS[0]['auid_neg'].each do |auid_neg|
      describe auditd_rules.syscall("#{sys_call}").fields.flatten do
  ...
  describe auditd_rules.syscall("#{sys_call}").fields.flatten do
    it { should include FCHOWNAT_AUDIT_FIELDS.first['key'] }
  end

  # Ensure all rule values are those that are expected
  describe auditd_rules.syscall("#{sys_call}").fields.flatten do
    it { should be_in FCHOWNAT_AUDIT_FIELDS.last }
  end
end
