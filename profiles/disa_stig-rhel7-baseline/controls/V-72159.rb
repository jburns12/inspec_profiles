...
SU_AUDIT_LINE = attribute(
  'su_audit_line',
  default: '^-a always,exit -F path=/bin/su -F perm=x -F auid>=1000 -F auid!=4294967295 -k privileged-priv_change',
  description: "The line that you use to audit su command"
)
...
The audit daemon must be restarted for the changes to take effect."
  ...
  describe auditd_rules do
    its('lines') { should match %r{#{SU_AUDIT_LINE}} }
  end
end
