...
describe.one do
    describe command("grep pam_faillock.so /etc/pam.d/password-auth-ac") do
      its('stdout') { should match /auth\s+required\s+pam_faillock.so .*even_deny_root.*\nauth\s+\[default=die\]\s+pam_faillock.so.*even_deny_root.*/}
    end
    describe command("grep pam_faillock.so /etc/pam.d/password-auth-ac") do
      its('stdout') { should match /auth\s+\[default=die\]\s+pam_faillock.so .*even_deny_root.*\nauth\s+required\s+pam_faillock.so.*even_deny_root.*/}
    end
  end
end
